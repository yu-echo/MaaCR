...
4. 模拟器设置里打开 **USB 调试**。
5. **把 MuMu 的「截图增强」关掉**（MuMu 设置 → 其他 / 性能 里）。
   MaaFramework 会自动探测设备支持哪种截图方式，而 MuMu 只上报「模拟器扩展」
   （`EmulatorExtras`）这一种；这条通道在部分 MuMu 版本上不可用，
   表现是点「开始任务」后日志停在「正在连接至模拟器……」、**一直没反应**。
   关掉它，框架就会退回普通 ADB 截图。详见「常见问题」。
...
| 现象 | 先看这里 |
| --- | --- |
| 点「开始任务」后停在「正在连接至模拟器……」，**一直没反应** | 多半是**截图方式**的问题。MuMu 只上报 `EmulatorExtras`（模拟器扩展）一种截图方式，而它在部分 MuMu 版本上不可用 —— 框架日志里会写 `cannot find any method to screencap!` / `failed to init screencap`，连接直接失败，而界面上只表现为「没反应」。**两种解法任选**：① MuMu 设置里**关掉「截图增强」**（推荐）；② MFAAvalonia「设置」里把**截图方式**从「自动」改成 `RawWithGzip` 或 `EncodeToFileAndPull`。这两种方式在 MuMu 12 上实测正常（连接 1.3s、截图 0.3s） |
