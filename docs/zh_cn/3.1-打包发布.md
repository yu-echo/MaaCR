...
`tools/validate.py` 会把这两件事都挡住：`pretask.exec` 写了**裸命令名**会报错
（写成相对路径是合法的，它按 `resource/base` 这个基准替你验存在性），
`resource/base` 不存在也会报错。

> `tools/validate.py` **在发布包里也能跑**（用包内自带的 Python）：
> `python\python.exe tools\validate.py`。它会自动认布局 ——
> 仓库是 `<repo>/assets/…`，发布包是 `<pkg>/…`。
> 这一条是发版后实测修出来的：原来写死了 `assets/`，在包里一跑就报
> 「找不到 assets\resource\pipeline」，而且**一个节点都没检查就宣布通过**，比报错更危险。
...
### 6.3 发出去之后，下载回来真跑一遍才发现的两件事（2026-09-23）

v0.1.0 发出去之后，把发布包下载到本机（真实 MuMu 环境）解压实跑，发现两个 bug——
**都是「离线怎么测都测不出来」的那类**：

1. **`tools/preflight.py` 找不到 MuMu 自带的 adb。**
   原来的候选只覆盖 `MuMuPlayer\shell\adb.exe` 这种旧布局，而 MuMu 12 的 adb 实际在
   `<根>\nx_main\adb.exe`（和 `MuMuManager.exe` 同目录）和 `<根>\nx_device\<版本>\shell\adb.exe`。
   找不到 adb 的后果不是报错，而是 `main()` 在那一步直接 `return 0` ——
   **「把模拟器拉起来」这半边功能完全不工作**，日志还只说「交给通用 UI 自己连设备」，
   很难看出是路径写错了。
   修法：先问已经找到的 `MuMuManager.exe` 要它旁边的 adb，再按安装根目录穷举两种布局。
   顺带把「MuMu 各版本目录结构会变」这件事写进注释。
2. **`tools/validate.py` 在发布包里跑不起来。**
   它把路径写死成仓库布局的 `assets/resource`，而发布包里是 `<包根>/resource`。
   README 里明明写着「发布包里也能跑 `python\python.exe tools\validate.py`」——
   一跑就是「找不到 assets\resource\pipeline」。
   **比报错更危险的是**：它一个节点都没检查，却照样把「检查模板引用：0 处」打完，
   看着像通过。
   修法：脚本自己认布局（有 `assets/interface.json` 就按仓库、否则按包根）。
   顺带修掉一个**假阳性**：它一开始把发布包里合法
   （相对 `resource/base`）的 `pretask.exec` 判成错误 —— 等于否定自己错误信息里写的「可行写法 ②」。

教训：**打包这类改动，「本地拼出来的目录」和「真正发布出去的产物」是两回事**，
一定要把发布出去的 zip 下回来解压实跑。这两个 bug 都只在真实产物 + 真实设备上才暴露。

### 6.4 顺带一提：GitHub 的 Release 附件下载要走代理

本机直连 `github.com/.../releases/download/...` 只有 ~1 MB/min（199 MB 要下三小时），
走本机 Clash 代理（`curl --proxy http://127.0.0.1:7897`）**28 秒就下完**。
奇怪的是 `api.github.com` 直连很快 —— 所以别看到 API 通就以为附件也通。
...
- [x] ~~发布包在**发布包布局**下解压后，结构与自带环境是否可用~~ —— **可用**。
      把发布出去的 zip 下载解压实跑：13 项关键路径全在、`interface.json` 的 4 个路径字段
      按 MFAAvalonia 的规则还原后都指向真实文件、自带 Python 是 3.11.9 且
      `maa` / `cv2 5.0.0` / `numpy 2.4.6` 都能 import。
- [x] ~~`pretask` 在**发布包布局**下能不能跑起来~~ —— **能**。按 MFAAvalonia 的方式
      （工作目录 `resource/base`、用包内自带 Python）实跑，13 秒把 MuMu 拉起来、
      adb 连上 `127.0.0.1:16384`、认出「国服皇室战争已安装 ✓」，退出码 0。
      ⚠️ 但这一步之所以能成，是因为先修了 6.3 里的 adb 路径 bug —— **v0.1.0 那个包里还是坏的**。
- [ ] 双击 `MFAAvalonia.exe` 真的能起图形界面（含缺 .NET 时的 `install-deps-win.bat`）
      —— 需要人点界面，没验
- [ ] **真正的对战/日常流程**：识别准不准、出牌逻辑、结算循环 —— 全程没在真机上跑过

> 也就是说：**「能跑起来」和「跑得对」是两件事**。上面打勾的都只证明前者。
